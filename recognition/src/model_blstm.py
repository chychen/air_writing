...
class HWRModel(object):
    ...
    def __init__(self, config, graph):
        ...
        self.input_ph = tf.placeholder(dtype=tf.float32, shape=[
            None, 647, self.input_dims], name='input_data')
        ...
        # Stacking rnn cells
        stack = tf.contrib.rnn.MultiRNNCell([cell] * self.num_layers,
                                            state_is_tuple=True)
        ...
        self.train_op = tf.train.MomentumOptimizer(
            learning_rate=0.005, momentum=0.9).minimize(self.losses_op, global_step=self.global_steps)

        # Option 2: tf.contrib.ctc.ctc_beam_search_decoder
        # (it's slower but you'll get better results)
        self.decoded_op, _ = tf.nn.ctc_greedy_decoder(logits, self.seq_len_ph)
...
def test_model():
    with tf.get_default_graph().as_default() as graph:
        ...
        X = np.ones([config.batch_size, 647,
