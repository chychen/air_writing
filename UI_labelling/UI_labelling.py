...
class SliderEngine(Widget):
    ...
    # Smoothe Line
    points = ListProperty([600, 200, 200, 200, 100, 400, 300, 500,
    ...
    def getCursorMatchedPoint_idx(self, cursor):
        normalizedX = cursor.center_x / (self.width - self.offset_x * 2)
        # one point refer to (x, y) in list
    ...
    def updateSelectedPoints(self):
        ...
        endPtIdx = self.getCursorMatchedPoint_idx(self.cursorEnd)
        print "startPtIdx", startPtIdx
        print "endPtIdx", endPtIdx
        self.selectedPoints = self.points[(startPtIdx)*2:(endPtIdx+1)*2]


class LabellingApp(App):
    """yeah
    """
    def build(self):
        slider = SliderEngine()
