...
class Touchtracer(FloatLayout):
    ...
    def on_touch_down(self, touch):
        ############## save trajectory to file ##############
        self.dict_trajectory['trajectory_list'] = list()
        #####################################################
    ...
    def on_touch_move(self, touch):
        ############## save trajectory to file ##############
        self.dict_trajectory['trajectory_list'].append(touch.sx)
        self.dict_trajectory['trajectory_list'].append(touch.sy)
        #####################################################
    ...
    def on_touch_up(self, touch):
        ############## save trajectory to file ##############
        if bool(self.dict_trajectory):
            # > 0 to avoid annoying linux mouse double click behavior
            collectedDataDir = 'testing_data/'
            dataName = str(datetime.now())
            # check file path exists
            if not os.path.exists(collectedDataDir):
                os.makedirs(collectedDataDir)
            fileName = collectedDataDir + dataName + ".json"
            f = open(fileName, 'w')
            json.dump(self.dict_trajectory, f)
            f.close()
            self.dict_trajectory.clear()
        #####################################################
        if touch.grab_current is not self:
            return
        touch.ungrab(self)
    ...
    def update_touch_label(self, label, touch):
        label.text = 'ID: %s\nPos: (%f, %f)\nClass: %s' % (
            touch.id, touch.sx, touch.sy, touch.__class__.__name__)
